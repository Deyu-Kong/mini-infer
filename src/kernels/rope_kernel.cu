 ...
 * [S, head_dim/2] (broadcast across batch and head dims).
 *
...
__global__ void rope_kernel(
    ...
    const __half* __restrict__ cos_t,    // [S, D/2]
    const __half* __restrict__ sin_t,    // [S, D/2]
    ...
    const __half* cos_row = cos_t + s * half;
    const __half* sin_row = sin_t + s * half;
...
__global__ void rope_precompute_kernel(
    ...
    const int64_t* __restrict__ positions, // [S]
    __half* __restrict__ cos_t,          // [S, half]
    __half* __restrict__ sin_t,          // [S, half]
    int S, int half) {
    const int s = blockIdx.y;
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= half) return;
    const float angle = static_cast<float>(positions[s]) * inv_freq[i];
    cos_t[s * half + i] = __float2half(cosf(angle));
    sin_t[s * half + i] = __float2half(sinf(angle));
}
...
void launch_rope_precompute(const float* inv_freq_h, const int64_t* positions_h,
    ...
    cudaMalloc(&positions_d, S * sizeof(int64_t));
    cudaMemcpyAsync(inv_freq_d, inv_freq_h, half * sizeof(float),
                    cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(positions_d, positions_h, S * sizeof(int64_t),
                    cudaMemcpyHostToDevice, stream);

    dim3 block(128);
    dim3 grid((half + 127) / 128, S);
    rope_precompute_kernel<<<grid, block, 0, stream>>>(
        inv_freq_d, positions_d, cos_d, sin_d, S, half);
